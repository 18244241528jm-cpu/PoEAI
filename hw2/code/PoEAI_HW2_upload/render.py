...
class Renderer():
    ...
    def _densify(self, mask_clone, mask_split, deterministic=False):
        ...
        Args: 
            mask_clone  :   (N, ) binary mask indicating which gaussian should be cloned.
            mask_split  :   (N, ) binary mask indicating which gaussian should be split.
            deterministic:  Bool, if True skip random perturbation for split positions.
        '''
        # Indices of parents
        ...
        # Split: create 2 copies per parent, scale down by 1.6
        ...
        # Copy 1 and Copy 2 for split (each with different position perturbation)
        new_means_s_1 = self.gaussians.means[idx_split].clone()
        new_means_s_2 = self.gaussians.means[idx_split].clone()
        if not deterministic:
            stds = torch.exp(new_scales_s)
            new_means_s_1 = new_means_s_1 + (torch.randn_like(new_means_s_1) * stds)
            new_means_s_2 = new_means_s_2 + (torch.randn_like(new_means_s_2) * stds)

        # Duplicate scales/quats/opacities/colors for 2 copies
        new_scales_s = new_scales_s.repeat(2, 1)
        new_quats_s = new_quats_s.repeat(2, 1)
        new_opacities_s = new_opacities_s.repeat(2)
        new_colors_s = new_colors_s.repeat(2, 1)
        new_means_s = torch.cat([new_means_s_1, new_means_s_2], dim=0)
        ### YOUR CODE ENDS HERE ###
        ...
        # Indices used to extend optimizer states (clone: 1 per parent, split: 2 per parent)
        ...
        zeros = torch.zeros((num_new, 1), device=self.device)
